...
<div class="d-flex justify-content-between align-items-center">
	<h5>Barang Temuan Anda</h5>
	<a href="{{ url('dashboard/find-item/create') }}"class="btn btn-primary btn-sm text-white"><i class="fa fa-plus"></i> Buat Penemuan</a>
</div>
<hr class="my-2">
...
<script>
	$(document).ready(function($) {
		...
		var firstSearchData = {'search_type': 'find_date', 'keyword': $("#filter-search input[name=name]").val(), 'date': $("#filter-search input[name=date]").val(), 'province_id': $('#select-province').val(), 'regency_id': $('#select-regency').val(), 'district_id': $('#select-district').val(), 'village_id': $('#select-village').val()};
		...
		$("#filter-search").on("submit", function(e){
			...
			sortSearch.push({name: 'search_type', value: 'find_date'});
